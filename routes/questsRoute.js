...
// a valid quest must be an object with essential quest attributes present and all quest attributes of the correct type
function validBody(questBody) {
  return (
    ...
    (typeof questBody.name === 'string'),
    (typeof questBody.nicknames === 'object') &&
    (questBody.nicknames.constructor === Array) &&
    (questBody.nicknames.filter((nickname) => typeof nickname != 'string').length === 0),
    (typeof questBody.talent === 'string'),
    (typeof questBody.contact === 'string'),
    (Number.isInteger(questBody.age)),
    (Number.isInteger(questBody.price)),
    (typeof questBody.rating === 'number'),
    (Number.isInteger(questBody.level))
...
router
  ...
  .get('/', function (req, res, next) {
    knex('quests')
    .join('dungeons', 'quests.dungeon_id', 'dungeon.id')
    .join('questgivers')
    // selectHeros()
